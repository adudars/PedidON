...
select * from pedido inner join item_cardapio on pedido.fk_id_itemcardapio = item_cardapio.id_itemcardapio order by hora;

select nome_funcionario,id_comanda,mesa from comanda inner join funcionario on comanda.fk_id_funcionario = funcionario.id_funcionario order by nome_funcionario;
